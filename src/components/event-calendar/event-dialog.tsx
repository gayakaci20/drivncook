  ...
  const handleSave = () => {
    ...
    if (!allDay) {
      ...
      if (startHours < StartHour || startHours > EndHour || endHours < StartHour || endHours > EndHour) {
        setError(`L'heure doit être comprise entre ${StartHour}:00 et ${EndHour}:00`)
        return
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{event?.id ? "Modifier l\'évènement" : "Créer un évènement"}</DialogTitle>
          <DialogDescription className="sr-only">
            {event?.id ? "Modifier les détails de cet évènement" : "Ajouter un nouvel évènement au calendrier"}
          </DialogDescription>
        </DialogHeader>
        {error && (
