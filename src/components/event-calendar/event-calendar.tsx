...
export function EventCalendar({
  ...
  const handleEventSave = (event: CalendarEvent) => {
    if (event.id) {
      onEventUpdate?.(event)
      toast(`Évènement "${event.title}" mis à jour`, {
      ...
      toast(`Évènement "${event.title}" ajouté`, {
        description: format(new Date(event.start), "d MMM yyyy", { locale: fr }),
        position: "bottom-left",
      })
    }
  ...
  const handleEventDelete = (eventId: string) => {
    ...
    if (deletedEvent) {
      toast(`Évènement "${deletedEvent.title}" supprimé`, {
  ...
  const handleEventUpdate = (updatedEvent: CalendarEvent) => {
    ...
    toast(`Évènement "${updatedEvent.title}" déplacé`, {
      ...
      return (
        <>
          ...
          <span className="max-md:hidden">
            {format(currentDate, "EEE d MMMM yyyy", { locale: fr })}
          </span>
        </>
      )
    } else if (view === "agenda") {
          ...
          <div className="flex items-center gap-2">
                ...
                <Button variant="outline" className="gap-1.5 max-[479px]:h-8">
                  <span>
                    <span className="min-[480px]:hidden" aria-hidden>
                      {({ month: 'M', week: 'S', day: 'J', agenda: 'A' } as any)[view]}
                    </span>
                    <span className="max-[479px]:sr-only">
                      {({ month: 'Mois', week: 'Semaine', day: 'Jour', agenda: 'Agenda' } as any)[view]}
                    </span>
                  </span>
                  <ChevronDownIcon
            ...
            >
              <PlusIcon
                ...
                aria-hidden="true"
              />
              <span className="max-sm:sr-only">Nouvel évènement</span>
            </Button>
          </div>
        </div>
