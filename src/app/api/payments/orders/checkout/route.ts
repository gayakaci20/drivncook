...
export async function POST(request: NextRequest) {
  try {
    ...
    const { orderId, successUrl, cancelUrl } = await request.json().catch(() => ({})) as { orderId?: string; successUrl?: string; cancelUrl?: string }
    ...
    if (!amountInCents || amountInCents <= 0) {
      return NextResponse.json({ success: false, error: 'Montant de commande invalide' }, { status: 400 })
    }
    ...
    const success = successUrl || `${baseUrl}/franchise/orders?payment=success&orderId=${order.id}&invoiceId=${invoice.id}`
    const cancel = cancelUrl || `${baseUrl}/franchise/orders?payment=cancelled&orderId=${order.id}`
    ...
    return NextResponse.json({ success: true, url: sessionCheckout.url, invoiceId: invoice.id })
  } catch (error) {
    console.error('Erreur checkout commande:', error)
